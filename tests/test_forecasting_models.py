...
class TestForecastingModels:
    ...
    def test_production_forecast_valid_data(self):
        """Test that ProductionForecast accepts valid data."""
        ...
        assert forecast.power_kw == 5.2
        assert forecast.energy_kwh == 5.2

    def test_production_forecast_optional_energy(self):
        """Test that ProductionForecast works without optional energy_kwh."""
        forecast = ProductionForecast(
            timestamp=datetime(2024, 1, 15, 10, 0, 0),
            power_kw=5.2
        )
        ...
        assert forecast.timestamp == datetime(2024, 1, 15, 10, 0, 0)
        assert forecast.power_kw == 5.2
        assert forecast.energy_kwh is None

    def test_production_forecast_invalid_negative_power(self):
    ...
    def test_forecast_response_negative_total_energy(self):
        ...
        assert "total_energy_kwh" in str(exc_info.value)
        assert "greater than or equal to 0" in str(exc_info.value)
